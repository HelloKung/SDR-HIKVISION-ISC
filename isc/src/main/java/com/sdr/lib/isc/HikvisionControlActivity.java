...
public class HikvisionControlActivity extends HikBaseActivity implements Interface.HikIscPlayCallback {
    ...
    public void onEnableVoice(int position, boolean enable) {
        if (enable) {
            AlertUtil.showPositiveToast("已开启");
        } else {
            AlertUtil.showPositiveToast("已关闭");
        }
    ...
    public void onControlSuccess(int position, String message) {
        hideLoadingDialog();
        AlertUtil.showPositiveToast(message);
    }
